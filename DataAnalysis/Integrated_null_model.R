...
code <- nimbleCode({
  ...
  for(i in 1:6){
    ...
    log(theta[i]) <- beta0[i] + mu.g0.star[i] * 11 + log(0.2) 
    pi.star[i] <- theta[i]/THETA
    phi.star[i] <- (phi[i] * sum(exp.beta0[1:(nspecies-1)]) * exp(mu.g0.star[i] * 11 + log(0.2)))/THETA
  ...
  for(i in 7:(nspecies-1)){
    ...
    log(theta[i]) <- beta0[i+1] + mu.g0.star[i] * 11 + log(0.2)
    pi.star[i] <- theta[i]/THETA
    phi.star[i] <- (phi[i] * sum(exp.beta0[1:(nspecies-1)]) * exp(mu.g0.star[i] * 11 + log(0.2)))/THETA
...
save(out.integrated, file = "./DataAnalysis/integratednullmodel_out.Rds")
